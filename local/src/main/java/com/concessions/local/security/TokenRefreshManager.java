...
public class TokenRefreshManager {
	...
	/**
	 * Initializes and starts the token refresh scheduler manually. Use
	 * 'synchronized' to ensure only one thread can start/stop the scheduler at a
	 * time. This method should be called after a successful login or connectivity
	 * check.
	 */
	@PostConstruct
	public synchronized void startRefreshScheduler() {
		/* HWC is this necessary?
		serverConfig.addPropertyChangeListener(evt -> {
			if (evt.getPropertyName().equals(ServerConfiguration.PROPERTY_TOKEN_RESPONSE)
					&& evt.getNewValue() != null) {
				TokenResponse tokenResponse = (TokenResponse) evt.getNewValue();
				if (StringUtils.isNotBlank(tokenResponse.refresh_token())
						&& StringUtils.isBlank(tokenResponse.access_token())) {
					Future<TokenResponse> future = authService.refreshToken(tokenResponse.refresh_token());
					try {
						tokenResponse = future.get();
