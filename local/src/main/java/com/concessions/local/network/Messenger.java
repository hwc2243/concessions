...
public class Messenger {
	...
	private String getServerIp () {
		return (StringUtils.isNotBlank(serverIp) ? serverIp : clientConfig.getServerIp());
	}
	
	private int getServerPort () {
		return (serverPort > 1023 ? serverPort : clientConfig.getServerPort());
	}
	
	public <T> T sendRequest (String service, String action, Object payloadObject, Class<T> responseClass)
			throws MessengerException {
		return sendRequest(getServerIp(), getServerPort(), service, action, payloadObject, responseClass);
	}
