...
public class ClientService {
	...
	public <T> T sendRequest (String service, String action, Object payloadObject, Class<T> responseClass) throws ClientException {
		...
		try {
			...
			throw new ClientException("Failed to serialize payload", ex);
		}
		...
		try (
				...
				if (response.isOk()) {
					...
					if (responseJson.isEmpty()) {
						...
						throw new ClientException("Server return ERROR but no message");
					}
					try {
						...
						throw new ClientException(responseObject.getMessage());
					} catch (JsonProcessingException e) {
						logger.error("Failed to deserialize ERROR response");
						throw new ClientException("Server returned ERROR but failed to parse response", e);
					}
				...
				throw new ClientException("Communications with server have failed", ex);
			}
	}
