...
public class LocalNetworkListener {
	...
	public LocalNetworkListener(HandlerRegistry registry, ObjectMapper mapper, boolean server) {
	...
	public boolean isComplete() {
		return tcpServerSocket != null && (!server || udpDiscoverySocket != null);
	}
	...
	public void start() throws IOException {
		...
		if (tcpServerSocket == null) {
			...
			findBindTcpPort(server ? serverConfig.getServerPort() : clientConfig.getClientPort());
			logger.info("{} bound successfully on {}:{}", listenerName, serverIp, tcpServerPort);
			if (server) {
				serverConfig.setServerIp(serverIp);
				serverConfig.setServerPort(tcpServerPort);
				clientConfig.setServerIp(serverIp);
				clientConfig.setServerPort(tcpServerPort);
				try {
				...
				clientConfig.setClientIp(serverIp);
				clientConfig.setClientPort(tcpServerPort);
				try {
					clientConfigService.save();
				} catch (BackingStoreException ex) {
					ex.printStackTrace();
				}
			}
		...
		if (server && udpDiscoverySocket == null) {
		...
		if (server) {
			logger.info("{} is active. IP Address: {}, TCP Port: {}, UDP Port: {}", listenerName, serverIp,
	 ...
	 */
	private void findBindTcpPort(int port) throws IOException {
		// Binding to port 0 tells the OS to assign any available port
		tcpServerSocket = new ServerSocket(port < 1024 ? 0 : port);
